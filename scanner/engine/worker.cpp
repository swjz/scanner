...
//  }
  ...
  // Task -> EvalWorkEntry
  std::map<i64, EvalWorkEntry> eval_map;
  while (true) {
    if (trigger_shutdown_.raised()) {
      // Abandon ship!
...
//          }
        ...
        derive_stencil_requirements_master(
            ...
            task_size_per_op, task_stream_map);

        derive_stencil_requirements_worker(
            meta, table_meta, jobs.at(new_work.job_index()), ops,
            analysis_results, job_params->boundary_condition(),
            table_id, job_id,
            std::vector<i64>(new_work.output_rows().begin(),
                             new_work.output_rows().end()),
            stenciled_entry, task_size_per_op);

        stenciled_entry.set_table_id(table_id);
